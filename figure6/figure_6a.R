################################################################################
###                              Figure 6A:                                  ###
####   Association between alpha diversity at M1 and Rotavirus-IgA at M6    ####
...
alpha.dat <- read.csv("figure6abc.csv") %>% as.data.table()

#Extract metadata
mapping<- alpha.dat %>% select(pid,sid,month,type,source,
                               feeding_method,feeding_method_m1,
                               bld.pca.weeks.m6,rec_age_weeks,
                               rvv.status.m6,log.titer.m6)

alpha.dat$sdepth <- rowSums(alpha.dat[,-(sid:log.titer.m6)])
...
tmp.p.alpha <- tmp.alpha[, apply(tmp.alpha, 2, function(x) sum(x>0)/length(x)) > 0.2] %>%
  as.data.frame() %>%
  dplyr::select(-s__human_gut, -s__uncultured_bacterium, -s__uncultured_organism) %>%
  {sweep(., 1, rowSums(.), "/")}
...
### Linear regression model adjusting for feeding method and postconception age. 
model <- lm(log.titer.m6~shannon+feeding_method_m1+bld.pca.weeks.m6,  data=meta.sh[!rvv.status.m6=="Unvaccinated"]) 
...
meta.sh  %>%
  ggplot(., aes(x=shannon,y=log.titer.m6)) +
  geom_point( size=5) +
  geom_smooth(method=lm, se=F, color= "#FF7256", size=1.2) +
  theme_bw() +
  scale_y_continuous(limits=c(-1,15))+
  labs(x="Shannon Diversity (M1)", y=expression(""*Log[2]*" Rotavirus-IgA Titer"))+
  annotate("label", x = 2.25, y = 15, 
           label = paste0("ß: ",gsub("\\.", "\u00B7", round(B1, 2)), "\np: ", gsub("\\.", "\u00B7", pval)), 
           hjust =0 , vjust = 1, size=8, color="#FF7256")+
