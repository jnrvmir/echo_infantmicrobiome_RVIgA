################################################################################
###                              Figure 6B:                                  ###
####        Overall microbiome composition (beta-diversity) at M1           ####
...
tmp.meta <- mdat %>% filter(sample_id %in% rownames(m1_tax_ra))

cov.ma <- mapping %>% filter(sid %in% rownames(m1_tax_ra)) %>%
  select(sid, bld.pca.weeks.m6, rec_age_weeks) %>%
  column_to_rownames(var = "sid") %>%
  as.matrix()

iga.level <- mapping %>% 
  filter(sid %in% rownames(m1_tax_ra)) %>% pull(log.titer.m6)

#create a distance matrix with bray curtis values
set.seed(164)

distm1.iga <- vegdist(m1_tax_ra, method = "bray", sample=100, iterations = 100) %>% as.dist()
## create distance kernel needed for MiRKAT
dist.kernel <- MiRKAT::D2K(as.matrix(distm1.iga))
