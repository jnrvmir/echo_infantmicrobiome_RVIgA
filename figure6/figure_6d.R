################################################################################
###                              Figure 6D:                                  ###
####   Association between alpha diversity and Rotavirus-IgA at M6    ####
...
##############################################################################
########################   Data Set Up  ###########################
alpha.dat <- read.csv("figure6def.csv") %>% as.data.table()

#Extract metadata
mapping<- alpha.dat %>% select(pid,sid,month,type,source,
                               feeding_method, bld.pca.weeks.m6,rec_age_weeks,
...
model <- lm(log.titer.m6~shannon+feeding_method+bld.pca.weeks.m6, data=meta.sh[!rvv.status.m6=="Unvaccinated"]) 
...
meta.sh %>%
  ...
  labs(x="Shannon Diversity (M6)", y=expression(""*Log[2]*" Rotavirus-IgA Titer (M6)"))+
  annotate("label", x = 2.75, y = 13, 
