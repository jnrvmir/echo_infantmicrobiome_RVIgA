################################################################################
###                              Figure 6C:                                  ###
####             Individual taxa at M1 association with                     ####
...
in.dat <- read.csv("figure6abc.csv") %>% as.data.table()
...
tmp.meta<- tmp.p.indv %>%
  ...
  select(sid, bld.pca.weeks.m6, rec_age_weeks) %>%
  column_to_rownames(var = "sid") %>%
  as.matrix()
...
test %>%
  ...
  mutate(taxa = gsub("g__", "", taxa), taxa = gsub("s__", "", taxa), 
         taxa = gsub("_", " ", taxa)) %>%
  mutate(taxonomy = recode(taxa,
                           "Streptococcus salivarius" = "Species",
                           "Klebsiella" = "Genus",
                           "Bifidobacterium" = "Genus",
                           "Collinsella" = "Genus",
                           "Atopobium"= "Genus",
                           "Peptostreptococcus anaerobius"= "Species",
                           "Escherichia Shigella" = "Genus",
                           "Lactobacillus"= "Genus",
                           "Schaalia radingae" = "Species")) %>%
  mutate(taxa = factor(taxa, levels = c("Schaalia radingae", "Atopobium", "Collinsella","Escherichia Shigella",
                                        "Lactobacillus","Peptostreptococcus anaerobius","Klebsiella", 
                                        "Bifidobacterium","Streptococcus salivarius"  ),
                       labels =  c("Schaalia\nradingae", "Atopobium", "Collinsella","Escherichia\nShigella",
                                   "Lactobacillus","Peptostreptococcus\nanaerobius","Klebsiella", 
                                   "Bifidobacterium","Streptococcus\nsalivarius"  ))) %>%
  
  separate(CI, into = c("ci_lower", "ci_upper"), sep = ", ", remove = FALSE) %>%
  mutate(ci_lower = as.numeric(gsub("[\\(]", "", ci_lower)), ci_upper = as.numeric(gsub("[\\)]", "", ci_upper))) %>%
  arrange(taxonomy, beta, taxa) %>%
  ggplot(., aes(x = beta, y = reorder(taxa, beta))) +
  geom_point(size = 4,color = "black") +
  geom_errorbarh(aes(xmin = ci_lower, xmax = ci_upper), height = 0.2, size=1.0,color = "black") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray40") +
  geom_text(aes(label = paste0("q = ", gsub("\\.", "\u00B7", signif(adjp, 2)))),
            nudge_y = 0.3,
            size = 5,
            color = "black") +
  scale_x_continuous(limits = c(-0.5, 0.5),labels = scales::label_number(decimal.mark = "\u00B7")) +
  labs(x = "Beta Estimate (Effect Size)",
       y = NULL) +
  theme_bw()  +
